...
const TabIcon = ({ focused, icon, title }: { focused: boolean; icon: any; title: string; }) => {
    if (focused) return (
        <ImageBackground
            ...
            className='flex flex-row w-full min-w-[112px] min-height-14 mt-4 justify-center items-center rounder-full overflow-hidden'
        >
            <Image source={icon} tintColor="#151312" className='size-5' />
            <Text className='text-seconday text-base font-semibold ml-2'>{title}</Text>
        </ImageBackground>
    )
    return (
        <View className='size-full justify-center items-center mt-4 rounder-full'>
